...
#ifndef RTXDI_RANKING_RESERVOIR_OFFSET
#define RTXDI_RANKING_RESERVOIR_OFFSET 3
#endif
...
// It is weightSum divide by the weight of selected sample (target pdf) * number of samples (M)
    ...
    RTXDI_ResamplingRuntimeParameters params)
{
    ...
    // We still need to samples a point inside the texel we sampled from pdf mipmap
    ...
    // Song: why it is the whole texture size instead of the texel size
...
// This controls how many pixels are going to sampling the sample tile pool,
// larger values would cause more correlation
    ...
    out RAB_LightSample o_selectedSample)
{
    ...
    for (uint i = 0; i < sampleParams.numLocalLightSamples; i++)
    {
        ...
        // Prepare light sample data based on the selected light and uv position
    ...
    out RAB_LightSample o_lightSample)
{
    ...
    // Start combining all the reservoirs into one
    RTXDI_Reservoir state = RTXDI_EmptyReservoir();
    ...
    inout RAB_LightSample selectedLightSample)
{
    ...
    // Ensure the current reservoir's M value not exceed the max value we can store in packed format
    ...
    {
        selectedLightPrevID = RAB_TranslateLightIndex(RTXDI_GetReservoirLightIndex(curSample), true); // return curSample.lightIndex
    }
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        if (tparams.enablePermutationSampling && i == 0) // false
        {
            RTXDI_ApplyPermutationSampling(idx, params.uniformRandomNumber);
        }

        if (!RTXDI_IsActiveCheckerboardPixel(idx, true, params)) // false
        {
    ...
    if (foundNeighbor)
    {
        ...
        {
            // Song: if remove this condition, what will happen?
            if (prevSample.age <= 1)
            {
        ...
        if (RTXDI_IsValidReservoir(prevSample))
        {
            // We already map the light index into current frame, so it is false
    ...
    inout RAB_LightSample selectedLightSample)
{
    ...
    for (i = 0; i < numSpatialSamples; ++i)
    {
        ...
        // If the neighbor sample is vaild, it's bit position will be 1, otherwise it is 0,
        // then we can track which samples is vaild in debiasing part
    ...
    if (RTXDI_IsValidReservoir(state))
    {
        ...
        if (sparams.biasCorrectionMode >= RTXDI_BIAS_CORRECTION_BASIC)
        {
            ...
            for (i = 0; i < numSpatialSamples; ++i)
            {
                ...
                // Song: What this M value will be? print this to see
    ...
    inout RAB_LightSample selectedLightSample)
{
    ...
    if (!stparams.enablePermutationSampling)
    {
        motion.xy += float2(RAB_GetNextRandom(rng), RAB_GetNextRandom(rng)) - 0.5; // This line always executes for now
    }
    ...
    for (i = 0; i < numSamples; ++i)
    {
        ...
        if (i == 0 && foundTemporalSurface)
        {
            ...
            idx = prevPos + spatialOffset; // Song: why do we need to add the offset again here?
        }
        else
        {
            uint sampleIdx = (startIdx + i) & params.neighborOffsetMask;
            ...
            // The heuristic here is for spatial offset check, the previous one is for temporal offset
            if (!RAB_IsSurfaceValid(temporalSurface))
                continue;
...
// This is for resampling both shading and ranking reservoirs
RTXDI_Reservoir RTXDI_SpatioTemporalResampling(
    uint2 pixelPosition,
    RAB_Surface surface,
    RTXDI_Reservoir curSample[2],
    RAB_RandomSamplerState rng,
    RTXDI_SpatioTemporalResamplingParameters stparams,
    RTXDI_ResamplingRuntimeParameters params,
    out int2 temporalSamplePixelPos,
    inout RAB_LightSample selectedLightSample[2],
    inout RTXDI_Reservoir restirRankingResult)
{
    if (stparams.biasCorrectionMode == RTXDI_BIAS_CORRECTION_PAIRWISE)
    {
        // return RTXDI_SpatioTemporalResamplingWithPairwiseMIS(pixelPosition, surface,
    ...
    // This can be shared for both reservoir since M value won't be clear to 0 for invisible sample
    uint historyLimit = min(RTXDI_PackedReservoir_MaxM, uint(stparams.maxHistoryLength * curSample[0].M));

    int2 selectedLightPrevID = -1;
    RTXDI_Reservoir state[2] = { RTXDI_EmptyReservoir(), RTXDI_EmptyReservoir() };
    for (uint j = 0; j < 2; j++)
    {
        if (RTXDI_IsValidReservoir(curSample[j]))
        {
            selectedLightPrevID[j] = RAB_TranslateLightIndex(RTXDI_GetReservoirLightIndex(curSample[j]), true);
        }

        RTXDI_CombineReservoirs(state[j], curSample[j], /* random = */ 0.5, curSample[j].targetPdf);
    }

    // Neighbor reusing sample position
    temporalSamplePixelPos = int2(-1, -1);
    uint startIdx = uint(RAB_GetNextRandom(rng) * params.neighborOffsetMask); // neighborOffsetMask = neighbor offset count
    ...
    for (i = 0; i < numSamples; ++i)
    {
        ...
        // If the neighbor shading data is similar, we mark this as valid
        cachedResult |= (1u << uint(i));

        uint2 neighborReservoirPos = RTXDI_PixelPosToReservoir(idx, params);

        // Two previous reservoirs
        RTXDI_Reservoir prevSample[2] = {
            RTXDI_LoadReservoir(params, neighborReservoirPos, stparams.sourceBufferIndex),
            RTXDI_LoadReservoir(params, neighborReservoirPos, stparams.sourceBufferIndex + RTXDI_RANKING_RESERVOIR_OFFSET)
        };

        // Fix the random number in combining for the same neighbor sample
        float combineRandom = RAB_GetNextRandom(rng);

        for (uint j = 0; j < 2; j++)
        {
            uint originalPrevLightID = RTXDI_GetReservoirLightIndex(prevSample[j]);

            prevSample[j].M = min(prevSample[j].M, historyLimit);
            prevSample[j].spatialDistance += spatialOffset;
            prevSample[j].age += 1;

            // Map the light ID from the previous frame into the current frame, if it still exists
            //  -> It does nothing for now
            if (RTXDI_IsValidReservoir(prevSample[j]))
            {
                if (i == 0 && foundTemporalSurface && prevSample[j].age <= 1)
                {
                    temporalSamplePixelPos = idx;
                }

                int mappedLightID = RAB_TranslateLightIndex(RTXDI_GetReservoirLightIndex(prevSample[j]), false);

                if (mappedLightID < 0)
                {
                    // Kill the reservoir
                    prevSample[j].weightSum = 0;
                    prevSample[j].lightData = 0;
                }
                else
                {
                    // Sample is valid - modify the light ID stored
                    prevSample[j].lightData = mappedLightID | RTXDI_Reservoir_LightValidBit;
                }
            }
            ...
            if (RTXDI_IsValidReservoir(prevSample[j]))
            {
                candidateLight = RAB_LoadLightInfo(RTXDI_GetReservoirLightIndex(prevSample[j]), false);

                candidateLightSample = RAB_SamplePolymorphicLight(
                    candidateLight, surface, RTXDI_GetReservoirSampleUV(prevSample[j]));

                neighborWeight = RAB_GetLightSampleTargetPdfForSurface(candidateLightSample, surface);
            }

            if (RTXDI_CombineReservoirs(state[j], prevSample[j], combineRandom, neighborWeight))
            {
                selected[j] = i;
                selectedLightPrevID[j] = int(originalPrevLightID);
                selectedLightSample[j] = candidateLightSample; // TODO: there should be two samples
            }
        }
    }

    // Start debiasing. Loop over two different reservoirs
    for (uint j = 0; j < 2; j++)
    {
        if (RTXDI_IsValidReservoir(state[j]))
        {
#if RTXDI_ALLOWED_BIAS_CORRECTION >= RTXDI_BIAS_CORRECTION_BASIC
            if (stparams.biasCorrectionMode >= RTXDI_BIAS_CORRECTION_BASIC)
            {
                // Compute the unbiased normalization term (instead of using 1/M)
                float pi = state[j].targetPdf;
                float piSum = state[j].targetPdf * curSample[j].M;

                if (selectedLightPrevID[j] >= 0)
                {
                    const RAB_LightInfo selectedLightPrev = RAB_LoadLightInfo(selectedLightPrevID[j], true);
                    ...
                    for (i = 0; i < numSamples; ++i)
                    {
                        ...
                        // Get the PDF of the sample RIS selected in the first loop, above, *at this neighbor*
                        const RAB_LightSample selectedSampleAtNeighbor = RAB_SamplePolymorphicLight(
                            selectedLightPrev, neighborSurface, RTXDI_GetReservoirSampleUV(state[j]));

                        float ps = RAB_GetLightSampleTargetPdfForSurface(selectedSampleAtNeighbor, neighborSurface);

#if RTXDI_ALLOWED_BIAS_CORRECTION >= RTXDI_BIAS_CORRECTION_RAY_TRACED
                                                                                                                // TODO:  WHY?
                        if (stparams.biasCorrectionMode == RTXDI_BIAS_CORRECTION_RAY_TRACED && ps > 0 && (selected[j] != i || i != 0 || !stparams.enableVisibilityShortcut))
                        {
                            RAB_Surface fallbackSurface;
                            if (i == 0 && foundTemporalSurface)
                                fallbackSurface = surface;
                        ...
                        // Select this sample for the (normalization) numerator if this particular neighbor pixel
                        //     was the one we selected via RIS in the first loop, above.
                        pi = selected[j] == i ? ps : pi;

                        // Add to the sums of weights for the (normalization) denominator
                        piSum += ps * prevSample[j].M;
                    }
                }

                // Use "MIS-like" normalization
                RTXDI_FinalizeResampling(state[j], pi, piSum);
            }
            else
#endif
            {
                RTXDI_FinalizeResampling(state[j], 1.0, state[j].M);
            }
        }
    }
    restirRankingResult = state[1];
